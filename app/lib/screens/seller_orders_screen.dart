...
class _SellerOrdersScreenState extends State<SellerOrdersScreen>
  ...
  Future<void> _updateStatus(
    ...
    // When confirming, set estimated ready time and delivery time
    if (newStatus == 'confirmed' && prepMinutes != null) {
      final now = DateTime.now();
      final readyAt = now.add(Duration(minutes: prepMinutes));
      // Add 30 min estimated transit on top of prep time
      final deliveryAt = readyAt.add(const Duration(minutes: 30));
      update['estimatedReadyAt'] = Timestamp.fromDate(readyAt);
      update['estimatedDeliveryAt'] = Timestamp.fromDate(deliveryAt);
    }
